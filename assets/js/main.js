...
function latLongToVector3(lat, long,hei) {
    var x,y,z,N,a,t;
    // var R =100000;
    // x = (R+hei) * Math.cos(lat) * Math.cos(lon)

    // y = (R+hei) * Math.cos(lat) * Math.sin(lon)
    
    // z = (R+hei) * Math.sin(lat)
    ...
    z = (((1-t*t) * N + hei) * Math.sin(lat))/1000 
    return new THREE.Vector3(x,y,z);
}
console.log(ver)
...
const material = new THREE.MeshNormalMaterial( { color: 0x888888 } );
